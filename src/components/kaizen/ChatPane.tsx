  ...
  return (
    <div className="flex h-full flex-col min-h-0">
      <div className="flex items-center gap-2 border-b border-black/10 p-2">
        ...
        {editing ? (
          <input value={title} onChange={e => setTitle(e.target.value)} onBlur={() => { setEditing(false); if (thread) renameThread(thread.id, title || 'Untitled'); }} className="flex-1 rounded bg-black/5 p-1" autoFocus />
        ) : (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {msgs.map(m => (
          <div key={m.id} className={`max-w-prose rounded px-3 py-2 text-sm ${m.role === 'user' ? 'ml-auto bg-green-500/20' : 'bg-black/5'}`}>
            <div>{m.content}</div>
      ...
      <form onSubmit={e => { e.preventDefault(); send(); }} className="border-t border-black/10 p-2 flex items-end gap-2">
        <textarea value={input} onChange={e => setInput(e.target.value)} aria-label="Message" onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !(e.nativeEvent as KeyboardEvent).isComposing) { e.preventDefault(); send(); } }} className="flex-1 rounded bg-black/5 p-2 text-sm" rows={2} />
