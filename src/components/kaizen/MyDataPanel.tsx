  ...
  return (
    <div className="space-y-4 text-sm">
      <div className="grid grid-cols-2 gap-2">
        <label><span className="block mb-1">Language</span><select value={profile.language} onChange={e=>setProfile({language:e.target.value as 'en'|'fr'})} className="w-full rounded bg-black/5 p-1"><option value="en">English</option><option value="fr">Français</option></select></label>
        <label><span className="block mb-1">Topic</span><input value={profile.topic} onChange={e=>setProfile({topic:e.target.value})} className="w-full rounded bg-black/5 p-1"/></label>
        <label className="col-span-2"><span className="block mb-1">Precise subject</span><input value={profile.preciseSubject||''} onChange={e=>setProfile({preciseSubject:e.target.value})} className="w-full rounded bg-black/5 p-1"/></label>
        ...
        <label><span className="block mb-1">Deadline</span><input type="date" value={profile.deadlineISO||''} onChange={e=>setProfile({deadlineISO:e.target.value})} className="w-full rounded bg-black/5 p-1"/></label>
        <label><span className="block mb-1">Weekly hours</span><input type="number" min={1} max={30} value={profile.weeklyHours} onChange={e=>setProfile({weeklyHours:Number(e.target.value)})} className="w-full rounded bg-black/5 p-1"/></label>
        <label><span className="block mb-1">Session length (min)</span><input type="number" min={15} max={120} step={5} value={profile.sessionLengthMin} onChange={e=>setProfile({sessionLengthMin:Number(e.target.value)})} className="w-full rounded bg-black/5 p-1"/></label>
      </div>
      <div>
        ...
        {profile.priorKnowledge.map((p,i)=>(<div key={i} className="mb-2 flex gap-2 text-xs"><input value={p.name} onChange={e=>updatePrereq(i,{name:e.target.value})} placeholder="Name" className="flex-1 rounded bg-black/5 p-1"/><input type="number" min={0} max={5} value={p.selfRatedMastery} onChange={e=>updatePrereq(i,{selfRatedMastery:Number(e.target.value) as ProficiencyScale})} className="w-14 rounded bg-black/5 p-1"/><input value={p.notes||''} onChange={e=>updatePrereq(i,{notes:e.target.value})} placeholder="Notes" className="flex-1 rounded bg-black/5 p-1"/><Button type="button" onClick={()=>removePrereq(i)} className="px-2 py-0 text-xs bg-red-500/30">×</Button></div>))}
      </div>
